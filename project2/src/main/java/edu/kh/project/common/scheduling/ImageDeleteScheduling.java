...
// @Component : 어떤 용도인지는 모르겠으나 일단 bean에 등록하겠다.
@Component
...
//    @Scheduled(cron = "0,10,20,30,40,50 * * * * *")// 매분 10초마다
...
//        System.out.println("0초마다 출력");
        ...
        // 3. 둘(dbList, fileList)을 비교하여 Server 이미지 목록 중 DB에 없는 이미지를 삭제
        if(!fileList.isEmpty()) { // 서버에 파일이 있다면
            
            for(File file : fileList) {
                
//                file.toString(); // 파일이 저장된 경로 반환
                // 문자열.subString(인덱스) : 저장된 인덱스부터 끝까지 잘라서 반환
                // lastIndexOf 사용
                ...
                //C:\Users\JeongHyun\eclipse-workspace\6_Framework\project2\src\main\webapp\resources\images\board\20221116105843_00001.jpg
                // >> /파일명.jpg 가 나옴  + 1 하면 파일명.jpg
