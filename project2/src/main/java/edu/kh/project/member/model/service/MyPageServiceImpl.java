...
public class MyPageServiceImpl implements MyPageService {
    ...
    // 프로필 이미지 수정
    @Transactional(rollbackFor = Exception.class)// 예외가 발생하면 롤백 // 업데이트햇으므로 트랜잭션처리가 필요함
    @Override
    public int updateProfile(String webPath, String filePath, MultipartFile profileImage, Member loginMember)
        ...
        // 중복 파일명 업로드를 대비하기 위해서 파일명 변경 관련
        String rename = null;
...
//        result = 0;
        ...
        if (result > 0) { // DB 수정 성공시 -> 실제로 서버에 파일 저장
            ...
            if (rename != null) {
                // 변경된 이미지명이 있다 == 새로운 파일이 업로드되어다
                profileImage.transferTo(new File(filePath + rename));
                // 메모리에 임시 저장된 파일을 지정된 경로에 파일형태로 변환 == 서버파일 업로드
