...
<body>
    <main>
        ...
        <%-- /board/4/2007/update --%>
            ...
            <c:forEach items ="${board.imageList}" var="img"> <%-- items에서 한개씩꺼내서 img라고 부르겠다. --%>
                <c:choose>
                    <c:when test ="${img.imageOrder == 0}"> <%-- 0이면 썸네일이다! --%>
                        <c:set var="thumbnail" value="${img.imagePath}${img.imageReName}" />
                                                        <%-- img.imagePath : 이미지가 있는 폴더경로나타남 --%>
                    </c:when>
