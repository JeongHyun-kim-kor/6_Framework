...
function loginValidate(){
    ...
    if(memberEmail.value.trim().length == 0){
        // 이메일 양쪽 공백을 제거한 후 길익 0인경우
        // == 이메일 미작성

        alert("이메일을 입력해주세요")

        memberEmail.focus(); //이메일input 요소에 초점을 맞춤
        memberEmail.value = ""; // 이메일 input 요소에 작성된 값을 모두 삭제
        return false;
    }

        // 비밀면호가 입력되지 않은 경우 false 반환

    if(memberPw.value.trim().length == 0){
        // 비밀번호 양쪽 공백을 제거한 후 길익 0인경우
        // == 비밀번호 미작성

        alert("비밀번호을 입력해주세요")

        memberPw.focus(); //비밀번호input 요소에 초점을 맞춤
...
selectEmail.addEventListener("click", (e) =>{
    ...
    $.ajax({
        ...
        type : "POST",
        dataType : "JSON", // 응답 데이터의 형식이 JSOn이다.-> 자동으로 JS객체로 전환
        success : (member) => {
            console.log(member);

            // 1. JSON 형태의 문자열로 반화된 경우(JSON -> JS 객체)
            // 방법 1) JSON.parse(문자열) 
            ...
            if(member == null){ // 일치 X
                ...
                //selectEmail()의 다음 요소로 추가(.after(요소))
