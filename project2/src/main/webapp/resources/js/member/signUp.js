    ...
    for(let key in checkObj){
        ...
        if(!checkObj[key]){
            ...
            switch(key){
                ...
                case "aythKey"          : str  = "인증이 완료되지 않았습니다.";        break;
            }
            alert(str); // 대화상자 출력
...
memberEmail.addEventListener("input", function(){
    ...
    if(regEx.test(memberEmail.value)) { // 유효한 경우
        ...
        // 선택적으로 data, type, dataType, success, error, complete, async등을
        // 포함시킬수 있다.
        ...
        $.ajax({
            ...
            success : (result) => {  // 비동기 통신을 성공해서 응답을 받았을때
                // result :서버로부터 전달받은 응답 데이터(매개변수 이름은 자유)
                console.log(result);
            ...
            error : () => {    // 비동기통신이 실패했을 때 수행
                console.log("ajax 통신 실패");
            },
            complete : () => {   // success, error 수행여부 관계없이 무조건 실행ㅑ
